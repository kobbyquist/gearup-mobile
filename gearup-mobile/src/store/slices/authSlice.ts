...
export const verifyRegistrationThunk = createAsyncThunk(
  ...
  async (data: { name: string; email: string; phone: string; password: string; role: 'OWNER' | 'MECHANIC'; code: string }, { rejectWithValue }) => {
    try {
      return await authService.verifyRegistration(data);
    } catch (error: any) {
      return rejectWithValue(error.message);
    }
  }
