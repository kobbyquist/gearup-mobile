...
export default function OwnerJobsScreen({ navigation }: any) {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch mechanic profiles for any assigned jobs we don't already have cached
  ...
  const handleCreateJob = async () => {
    ...
    try {
      ...
      // For direct requests, send an expandable job card into chat with that mechanic
      if (requestType === 'DIRECT' && selectedMechanic) {
        try {
          const existing = await messageService.getConversationWithUser(user?.userId, selectedMechanic.id).catch(() => null);
          ...
          await messageService.sendJobCard(chatJobId, user?.userId, selectedMechanic.id, meta);
        } catch (cardError) {
          // Job was created successfully even if the chat card failed to send — don't block the success flow
        }
      }
  ...
  const handleDatePickerDone = () => {
    if (form.scheduledDate && !validateAndSetSchedule(form.scheduledDate)) {
      return;
    }
    setShowDatePicker(false);
  };
  ...
  // Group filtered jobs into sections by recency
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Create Job Modal — form / mechanic picker / map picker as inline views (single Modal, no nesting) */}
      <Modal visible={modalVisible} animationType="slide" presentationStyle="pageSheet">
        ...
        <View style={styles.modal}>
                      ...
                      <View key={m.id} style={styles.mechanicPickCard}>
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            ...
                            setForm(prev => ({ ...prev, scheduledDate: null }));
          ...
          {/* In-app alert overlay (validation errors, warnings, GPS success) — covers whichever inner view is active */}
          {formAlert && (
          ...
          {/* In-app job-created success overlay */}
          {jobCreatedSuccess && (
            <View style={styles.inModalOverlay}>
...
const styles = StyleSheet.create({
  ...
  availabilityHintWarnText: { fontSize: FONT_SIZES.xs, color: '#b45309', flex: 1 },
  ...
  paymentOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: SPACING.xl },
  paymentCard: { backgroundColor: '#fff', borderRadius: RADIUS.lg, padding: SPACING.xl, width: '100%', alignItems: 'center' },
  paymentIconWrap: { width: 60, height: 60, borderRadius: 30, backgroundColor: '#f0fdf4', justifyContent: 'center', alignItems: 'center', marginBottom: SPACING.md },
  paymentTitle: { fontSize: FONT_SIZES.lg, fontWeight: '700', color: '#1b1b1b', marginBottom: 4, textAlign: 'center' },
  paymentSubtitle: { fontSize: FONT_SIZES.sm, color: '#6b7280', textAlign: 'center', marginBottom: SPACING.lg },
  paymentOption: { flexDirection: 'row', alignItems: 'center', gap: SPACING.md, alignSelf: 'stretch', padding: SPACING.md, backgroundColor: '#f9fafb', borderRadius: RADIUS.md, marginBottom: SPACING.sm, borderWidth: 1, borderColor: '#f3f4f6' },
  paymentOptionIconWrap: { width: 38, height: 38, borderRadius: 19, justifyContent: 'center', alignItems: 'center' },
  paymentOptionText: { flex: 1, fontSize: FONT_SIZES.md, fontWeight: '600', color: '#1b1b1b' },
  paymentCancelBtn: { marginTop: SPACING.sm, padding: 12, alignItems: 'center' },
  paymentCancelText: { fontSize: FONT_SIZES.md, fontWeight: '600', color: '#9ca3af' },
});